...
def train(mnist):
    ...
    with tf.Session() as sess:
        ...
        '''
        在训练过程中不再测试模型在验证数据上的表现，验证和测试的过程会有一个
        独立的程序来完成。
        '''
        for i in range(TRAINING_STEPS):
            ...
            if i%1000 == 0:
                '''
                输出当前的训练情况。这里只输出了当前训练batch上的损失函数大小
                通过损失函数的大小可以大概了解训练的情况。在验证数据集上的准确率会
                有一个单独的程序来完成。
                '''
                print("After %d training step(s),loss on training batch is %g"%(step,loss_value))
                '''
                保存当前的模型。注意这里给出了global_step参数，这样可以让每个被保存模型的文件末尾加上训练的轮数，
                比如，“modal.ckpt=1000”表示训练1000轮之后得到的模型。
                '''
